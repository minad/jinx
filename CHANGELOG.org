...
- Rename =jinx-correct-next/previous= to =jinx-goto-next/previous=. The commands can
  be used outside the minibuffer.
- Bind =M-n= and =M-p= to =jinx-goto-next/previous= in =jinx-misspelled-map=.
