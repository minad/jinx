...
Most importantly your Emacs must be compiled with dynamic module support.
Furthermore Jinx requires =libenchant=. Enchant library is a required dependency
for Jinx to compile its module at install time. If =pkgconf= or =pkg-config= is
available, Jinx will use it to locate =libenchant= during installation. Depending
on your BSD or Linux distribution you have to install different packages:
