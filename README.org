...
Jinx, Jinx will use =pkg-config= to locate =libenchant=. Depending on your operating
system and Linux distribution you have to install different packages:

- Debian or Ubuntu: =libenchant-2-dev=, =pkg-config=
- Fedora or RHEL: =enchant2-devel=
- Mac: =enchant2=, =pkgconfig=
